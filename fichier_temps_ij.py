...
class FichierTempsIJ(QgsProcessingAlgorithm):
    """
    ...
    def processAlgorithm(self, parameters, context, feedback):
                ...
                if texte==True:
                    ...
                    if tc_seul==False or float(elements[cols['ligne']])>0:
                        ...
                        if elements[cols['origin']] not in links[elements[cols['ij']]]:
                                links[elements[cols['ij']]][elements[cols['origin']]]=[elements[cols['ij']],float(elements[cols[variable]]),1.0,float(elements[cols[variable]]),float(elements[cols[variable]]),elements[cols['pole']],elements[cols['pole']],[elements[cols['heureo']]],[elements[cols['heured']]],float(elements[cols[variable]])**2,elements[cols['origin']],elements[cols['origin']],float(elements[cols['ti']]),float(elements[cols['ti']])]
                        else:
        ...
        res.write('ij;jour;avg;nb;temps;max;pole_min;pole_max;departures;arrivals;sdev;o_min;o_max,ti,avg_i\n')
        for j,i in enumerate(links):
            ...
            try:
                sum_tri=sum_tri[0][1]
                res.write(sum_tri[0]+";"+unicode(sum_tri[1]/sum_tri[2])+";"+unicode(sum_tri[2])+";"+unicode(sum_tri[3])+";"+unicode(sum_tri[4])+";"+sum_tri[5]+";"+sum_tri[6]+";"+unicode(len(sum_tri[7]))+
                        ";"+unicode(len(sum_tri[8]))+";"+unicode((abs(-((sum_tri[1]**2)/sum_tri[2])+sum_tri[9]))**0.5)+";"+sum_tri[10]+";"+sum_tri[11]+unicode(sum_tri[12])+";"+unicode(sum_tri[13])+"\n")
            except:
                pass
    ...
    def displayName(self):
        ...
        """
        return self.tr('indicators by link IJ')
    ...
    def tr(self, string):
        return QCoreApplication.translate('FichierTempsIJ', string)
